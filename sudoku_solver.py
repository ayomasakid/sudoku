...
def batch_smart_solve(grids, solver, confidence_threshold=0.5):
    """
    ...
    confidence_threshold (float): Threshold for confidence in predictions
    ...
    for _ in range((grids == 0).sum((1, 2)).max()):
        # Print or inspect the shape of the grids before prediction
        print("Grids shape before prediction:", grids.shape)
        
        preds = np.array(solver.predict(to_categorical(grids)))  # get predictions
        
        # Print or inspect the shape of preds
        print("Predictions shape:", preds.shape)
        
        probs = preds.max(2).T  # get the highest probability for each 81 digit to predict
        ...
        for grid, prob, value, zero in zip(grids, probs, values, zeros):
            if any(zero):  # don't try to fill already completed grid
                ...
                confidence_position = where[prob[zero] > confidence_threshold]  # best score FOR A ZERO VALUE (confident blank)
                if len(confidence_position) > 0:
                    confidence_position = confidence_position[0]
...
solver.save("sudoku_solver_model.h5")